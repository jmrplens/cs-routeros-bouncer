...
- **The benchmarking page plots the full lifecycle at 100 ms resolution** — a single continuous 51-second capture from the production RB5009: idle with the bouncer stopped and the list empty, the first reconciliation importing ~22,000 entries (a ~31% CPU plateau for ~26 s, single-core slices to 100%), and the quiet state after. Two mermaid line charts (CPU %, RAM MiB — mermaid has no second Y axis), themed from the palette: xychart derives nothing from the shared theme stops, and without an explicit block its plot line shipped as an invisible cream on white. The stale numbers the new instrument corrected: the reconcile transient is a ~2 s plateau at ~31%, not "about six seconds at 29–34%"; a restart with all entries present takes ~7–8 s, not ~75–77 s; steady reconciliation is ~1.9–2.1 s, not ~3–4 s; and the RAM step (~19 MiB) lands at connection time, with 22,000 entries adding nothing measurable after it
