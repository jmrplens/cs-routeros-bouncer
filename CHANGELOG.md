...
- **A social card per documentation page** — all 56 pages shared one `og-image.png`, so a link to the reconciliation internals and a link to the Spanish CLI reference previewed identically. `src/pages/og/[...slug].png.ts` renders one per page at build time from the same mark and palette as every other asset, with the title set in the mono face the site already uses — which is what makes wrapping exact arithmetic rather than a guess, since librsvg does not measure text. `check-social-cards.mjs` joins the two halves by reading every `og:image` out of the rendered HTML and asking the filesystem whether it is there
