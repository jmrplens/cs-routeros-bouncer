...
- **The Go tooling is standardized against the reference repo** — `make analyze` becomes an accumulator that runs all twelve steps even when one fails and names each failure, instead of a prerequisite chain that dies on the first; it also gains `golangci-lint config verify`, which immediately caught the local stale-toolchain trap. New targets: `analyze-fix` (every automatic fix the tools offer), `sonar`/`sonar-status` (the full SonarCloud pipeline locally via `scripts/sonar-scan.sh`, gate + conditions + measures), `hadolint`, `release-check` and `version`. CI gains a hadolint job over both Dockerfiles; their HEALTHCHECKs move to exec form — busybox wget exits 0/1 across every failure mode (verified empirically on alpine 3.24 and in the built image), so the traditional `|| exit 1` had nothing to normalize and no ignore was needed. The `.golangci.yml` needed nothing: it was already the reference config, 53 linters to the reference's 52. The `bin/tools` stamp machinery is gone: tools resolve from PATH and `make tools` installs the pinned versions with the system Go, so every binary is built by the same toolchain go.mod targets
