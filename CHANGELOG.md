...
- **The benchmarking page plots the full lifecycle at 100 ms resolution, and the instrumentation installs itself** — a single continuous 70-second capture from the production RB5009 (idle with the bouncer stopped and the list empty, the first reconciliation importing ~22,000 entries at a ~31% CPU plateau, the quiet state after) drawn as one dual-axis chart: CPU % left, RAM MiB right, dashed markers at the bouncer's own log timestamps. Drawn by `cmd/perfmon plot` as deterministic SVG pairs from a committed CSV — mermaid's xychart has neither a second Y axis nor annotations, and matplotlib would add a Python dependency whose output shifts between versions. `cmd/perfmon install` deploys the whole 10 Hz sampler (a scratch container reading the shared kernel's /proc/stat) to any RouterOS device in one command: cross-build, dockerless image tar, veth + NAT + the two firewall-list memberships that otherwise silently eat the container's traffic, and an end-to-end verify against Loki. The stale numbers the instrument corrected: the reconcile transient is a ~2 s plateau at ~31%, not "about six seconds at 29–34%"; a restart with all entries present takes ~7–8 s, not ~75–77 s; steady reconciliation is ~1.9–2.1 s, not ~3–4 s; and the ~19 MiB RAM step lands at connection time, with 22,000 entries adding nothing measurable after it
- **`mikrotik.command_timeout` does something now** — the key was parsed, defaulted to 30s, mapped to `MIKROTIK_CMD_TIMEOUT`, documented with tuning advice, and read by nothing: a router that accepted the TCP connection and then stalled held a command forever. It now arms a connection deadline around each whole command (write + reply) in the vendored client, cleared between commands so idle gaps can never trip it. Pinned by three tests: a stalled reply fails near the configured bound, an idle gap longer than the bound does not fail the next command, and zero keeps the old unbounded behavior
- **A social card per documentation page, on a dark ground** — all 56 pages shared one white `og-image.png`, so a link to the reconciliation internals and a link to the Spanish CLI reference previewed identically, on a sheet of white that reads as a placeholder in feeds that are themselves dark. Cards now take the dark palette (the icons keep the light one, since a launcher does not follow this site's theme) over a ground built from the mark's own statement: lanes arriving as dashed approaches, three crossing an indigo rail and one stopping at it. Title contrast measures 15.4:1 and the muted lines 4.81:1 on the rendered card. `src/pages/og/[...slug].png.ts` renders one per page at build time from the same mark and palette as every other asset, with the title set in the mono face the site already uses — which is what makes wrapping exact arithmetic rather than a guess, since librsvg does not measure text. `check-social-cards.mjs` joins the two halves by reading every `og:image` out of the rendered HTML and asking the filesystem whether it is there
- **Four architecture diagrams** — the rule-placement ladder, the decision funnel in three swimlanes, the reconciliation diff, and the shutdown order — under one written convention: a solid edge is a write, a dashed edge is a read. Three channels carry it (line pattern, then colour, then a legend inside each diagram) so a diagram survives greyscale, colour vision deficiency and a printout. The existing overview flowchart drew all six of its edges solid, including the decision stream the bouncer *reads*; it now follows the same rule as the page that describes it
...
- **The RouterOS API client is vendored, pruned and repaired** — upstream `go-routeros/routeros` has had no maintainer activity for eighteen months and no maintained alternative exists, so v3.0.1 now lives in `internal/rosapi` (MIT, provenance in its README). The async/listen mode — which nothing in this repository ever called — is removed, and with its `Cancel()` gone the reader stops dispatching every `Read` to a fresh goroutine: at production scale that layer cost ~294,000 goroutine spawns and ~62 MB of garbage per reconcile cycle. The rewrite is pinned by a committed fingerprint test over a 22,037-entry reply and error-shape tests; upstream's own suite passes under `-race`. Parse allocations drop from ~101 to ~25 per address-list entry
- **The reconciliation cycle stops fetching two dead properties** — `ListAddresses` asked the router for five properties per entry; `list` is whatever the query just pinned with `?list=`, and `timeout` is never read off an entry that came from the router. Measured on an RB5009 holding 22,037 entries, the two prints are 98.5% of the cycle, and dropping the two properties takes the median reconcile from 2.153 s to 1.913 s (−11.2%), with every one of 48 paired comparisons favouring the new binary and no overlap between the ranges. Wire cost per IPv4 pass falls from 4.07 MB to 3.14 MB. Half the cycle is a floor this does not reach: a `count-only` print of the same list still costs 1.18 s
- **The web manifest and the browser chrome follow the palette** — `theme_color`, `background_color` and the `theme-color` meta were all `#0e1316`, matching the dark `--rb-page` by coincidence rather than by construction. The meta now comes from the palette astro.config already parses, and is split in two: one value painted a dark address bar above a white page for every light-theme reader. A new `icon-maskable-512.png` gives Android an adaptive icon — without one it does not crop to the launcher shape, it shrinks the mark onto a plain white tile — and the manifest is asserted against the tokens, against the existence of every icon it names, and against having a maskable entry at all
- **A new mark, and the palette moves to CrowdSec's indigo** — the shield was replaced. Rendered side by side at the sizes that matter it was, unmistakably, the WiFi glyph: a shield containing concentric arcs radiating from a dot is the standard signal icon at 104px and at 23px, which for a network tool is worse than generic. The drawing that replaced it is four lanes meeting a rail — three cross and continue, one stops dead at it — chosen from twenty-nine candidates across two waves and unanimous on every judging lens. The proportion is deliberate: a firewall does not block everything, it blocks the exception. Three primitives, 282 bytes of geometry, no ids, no defs, no mask.

  The accent moves from teal to `#4d4a98`, CrowdSec's own indigo, with `#807dc0` for the dark theme — the same hue and saturation raised until it clears 4.5:1 against the *surface*, not merely the page, because the terminal prompt glyph and the sidebar pill both sit there and the page-only value failed both. This is ecosystem alignment rather than the co-branding retired earlier: the previous palette paired CrowdSec indigo *with* MikroTik teal, implying endorsement by two vendors; this is a CrowdSec bouncer, listed on their Hub, and looking like one is accurate.

  The rail takes the accent because the boundary is what the product is. Pass and stop are carried by LENGTH before colour, which matters: the accent and the muted tone measure 1.01:1 apart in the light theme, so a mark relying on that pair to distinguish them would collapse into one tone in greyscale
- **The brand mark is painted from theme tokens, and the rasters are rendered from it** — the two SVGs were byte-identical except four hex literals in a palette no stylesheet declared, so the contrast gate could not see them and the mark had already drifted. The mark now carries class hooks only. `docs/scripts/render-brand-rasters.mjs` renders the favicon, both app icons, the Apple touch icon and the social card from that one file, resolving the same tokens out of `theme.css`, and `--check` fails the build when any is stale — wired into `pnpm analyze`. This closes the gap that let the site ship a header in one palette and a favicon, app icon and social card in the retired CrowdSec-indigo/MikroTik-teal pair
...
- **`favicon.ico` was still the retired shield** — every other icon is rendered from `logo-light.svg`, but the `.ico` was not: it was last written by the July logo redesign and carried the shield-with-WiFi-arcs through the whole mark change. It is the one asset the author never looks at and the one Google's SERP fetcher and older Safari prefer, so the search result showed a logo the site had retired. It is now packed by `render-brand-rasters.mjs` from the same PNGs as everything else, at 16/32/48 — by hand, because sharp cannot encode ICO and an ImageMagick dependency would not survive CI. 15,086 bytes to 553
- **The home page's social card 404'd** — the card URL is assembled in two places holding different ids for the same page: the endpoint reads the content collection, which calls the English home page `index`, while the `Head` override sees Starlight's route, whose id for it is empty. They agreed on 55 pages and disagreed on the one most links point at, which shipped as `og/.png`. Starlight also synthesises a 404 route with an entry and no collection page behind it, so that page pointed at a card nothing rendered. Both now resolve through one `cardPath`, and membership in the collection decides whether a page has a card at all
- **The print stylesheet shipped a 1.61:1 boundary** — `@media print` was exempt from the contrast gate on the grounds that paper is one background and the palette does not reach it. The first half is true; the second does not follow. Browsers do not print background colours by default, so on paper the `pre` border is the only thing separating a code block from the prose around it — a graphical object required to understand the content, which puts it inside WCAG 1.4.11 rather than outside it. Now `#8a8a8a` at 3.45:1, read out of the stylesheet by the gate rather than restated in it
- **The i18n gate invented one mismatch and missed two** — `path="a.b"` and `path={"a.b"}` are the same invocation written two ways and were keyed apart, so twins that agreed were reported as differing; the self-test asserted that behaviour in two places, so the bug was pinned by its own suite. `<Home section />` and `<Home />` keyed identically, because a valueless attribute was indistinguishable from an absent one. A stray `<Foo-Bar />` in a `.md` page was reported as `Foo`, naming a component that does not exist
- **The drawing convention now holds across every architecture diagram** — six accuracy defects were corrected against the Go source, and the overview page's sequence diagram, which drew every message solid including the ones the bouncer reads, now distinguishes them. One claim was rewritten rather than removed: entries whose comment does not match the prefix were described as "never listed", but `ListAddresses` queries on the list name alone and filters client-side, so they are fetched on every pass and only then dropped — which costs transfer an operator should know about
