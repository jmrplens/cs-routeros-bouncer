...
No cost model produces that shape. Re-run as three interleaved rounds reporting the best time per size, it flattens to 4.5–6.1 s across the whole 20× range — which is the real finding: the chunk size barely matters, because the per-entry insertion cost dominates the per-chunk scaffolding by more than a hundred to one.
