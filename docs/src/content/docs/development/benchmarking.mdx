...
reading the shared kernel's `/proc/stat` at 10 Hz — and `/proc/meminfo` once
per second, since memory does not move at 100 ms on this workload — and
...
container and verifies that samples reach Loki. `capture` pulls any window as
CSV; `plot` redraws the chart above from the committed dataset. Run any verb
with `-h` for the full flag list.

`uninstall` removes exactly what `install` created and nothing else: every
object is selected by the exact comment tag `install` wrote, never by a
pattern — the address-list table it touches is the one the bouncer keeps its
blocking state in, so a substring match there was never acceptable — plus the
uploaded image file. It then re-runs its own ownership checks and fails,
naming the step, if anything remains. A hand-made setup that happens to share
a name is left alone on both sides: `install` refuses to proceed and names the
collision — it does not build on objects it did not create and will not later
remove — and `uninstall` does not own it and ignores it. The envlist and the
uploaded image, which RouterOS cannot tag, are signed by a `PERFMON_TAG`
entry in the envlist and are removed only while that marker exists.

Two things about the deployment are deliberate and worth knowing before you
rely on it. The container's root lives on the router's tmpfs by default and it
is registered with `start-on-boot=no`, so it does **not** survive a reboot —
an instrument should leave no trace on a device it was only borrowing;
`install` is idempotent and fast to rerun. And the installer makes two
firewall-list memberships because the default RouterOS firewall silently
eats container traffic without them: the raw rule `drop the rest
(in-interface-list=!LAN)` drops every packet from a veth that is not in the
`LAN` interface list, and `drop local if not from default IP range` matches a
source outside the `LANs` address list. The symptom is that ICMP to the
container works while its outbound TCP produces no conntrack entry and no
masquerade counter; the sniffer shows nothing. Both memberships are created
