...
1. **Verify** `rule_placement` in configuration. Use `"top"` for highest priority or `"bottom"` to append. Use structured placement (`position`, `before_comment`, `after_comment`) when existing router rules must stay first.
