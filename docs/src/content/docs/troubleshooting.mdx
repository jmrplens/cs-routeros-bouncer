...
3. CPU impact is temporary — only while reconciliation is actively adding or removing entries

### Sustained high RouterOS CPU after startup

:::danger[Router CPU stays high after reconciliation]
...
If you still see repeated duplicate errors and reconnect attempts, upgrade to a build that includes the RouterOS device-error handling and cache fast-path fixes.
