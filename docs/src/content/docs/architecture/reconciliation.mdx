...
| No-drift periodic check        | ~1.9 s, with no add/remove writes                  |
| CAPI → local-only mass removal | ~26,800 removals in ~77 s of RouterOS removal work |

The no-drift figure is the one that has moved: it was ~3–4 s when this table was
first measured, and two changes since then account for the difference — the
reconcile pass stopped fetching two address-list properties nothing reads
(−11.2%), and an individual unban stopped traversing the list to rediscover an
id it already had. The current value comes from the bouncer's own
`operation_duration_seconds` histogram on a production RB5009 (RouterOS 7.24.1,
22,332 IPv4 entries plus ~525 IPv6, 60 s interval): **1.86 s over 28 consecutive
cycles**, a 3.1% duty cycle against wall-clock. Of that, the two address-list
prints are 98.5%.
