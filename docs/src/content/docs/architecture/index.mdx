...
The bouncer maintains a configurable pool of persistent RouterOS API connections. During reconciliation, work is distributed across the pool using the generic `ParallelExec` helper; on an RB5009 with `pool_size: 10`, full CAPI bulk-add work for ~28,700 entries completed in ~35–36 s.
