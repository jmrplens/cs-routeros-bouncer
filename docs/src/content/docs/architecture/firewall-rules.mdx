...
With `rule_placement: "top"`, the bouncer places managed rule blocks at position 0 for maximum priority. If a dynamic/built-in rule occupies position 0, the bouncer iterates through positions until finding a valid placement. Structured placement can also target a zero-based position or an existing rule comment.
