...
flowchart TB
    ...
    subgraph LAPI["Lane 1 · CrowdSec LAPI, server side"]
        ...
        ALL -. "type: snapshot only, default set" .-> N1
    ...
    subgraph BOUNCER["Lane 2 · Bouncer, in process"]
        ...
        SENT -. "expiry" .-> T
        T -. "no" .-> N2
        T -. "yes, duration unparseable" .-> W["Warned, enforced as 4h"]
        T -. "yes" .-> PR{"family enabled?"}
        W -.-> PR
        PR -. "no" .-> N2
        PR -. "ban" .-> CA{"in the cache?"}
        PR -. "expiry" .-> CD{"in the cache, and still on the router?"}
