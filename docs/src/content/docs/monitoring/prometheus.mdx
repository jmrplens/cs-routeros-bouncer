...
<Aside type="note">
Processed traffic is typically 200-500x higher than dropped traffic, since it counts ALL packets entering the bouncer chains — not just the ones matching banned IPs. This ratio is expected and provides context for how much traffic the bouncer is actually evaluating.
</Aside>
...
<Aside type="note">
Dropped and processed metrics are sent as deltas (difference since the last push). The LAPI aggregates these to show cumulative bouncer activity in `cscli metrics show bouncers`. The `ip_type` label enables per-protocol analysis in the CrowdSec Console.
</Aside>
