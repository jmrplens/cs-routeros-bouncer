...
description: Complete reference of every Prometheus metric exported by cs-routeros-bouncer, with labels, example PromQL queries, and the LAPI usage-metrics push.
---
...
| Label    | Values                                            | Description                                                                                                                                                                                                                               |
| -------- | ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `origin` | `crowdsec`, `cscli`, `CAPI`, `lists`, `reconcile` | Source of the decision, passed through from CrowdSec verbatim. Blocklist subscriptions all report the literal `lists`, whatever the list is named. `reconcile` is the bouncer's own label for entries re-applied by a reconciliation pass |
...
<Aside type="note">
  ...
  the bouncer is actually evaluating. The ratio was observed on the [benchmark
  platform](/cs-routeros-bouncer/development/benchmarking/) and varies with your
  traffic mix.
</Aside>
