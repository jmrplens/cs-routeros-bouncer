...
<Badge text="Labels: ip_type" variant="default" />
...
<Badge text="Labels: action, origin, ip_type" variant="default" />
...
<Badge text="Counter" variant="success" />
<Badge text="Labels: component" variant="default" />
...
`firewall_rule_placement` is a configuration label. Simple values appear as `top` or `bottom`; structured placement appears as a compact summary such as `position:5`, `after_comment:drop invalid`, or `top,filter=after_comment:drop invalid,raw=position:2`.
