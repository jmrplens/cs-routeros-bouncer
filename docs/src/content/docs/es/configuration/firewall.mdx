---
title: Firewall
description: "Configura cómo bloquea el tráfico cs-routeros-bouncer en MikroTik: listas IPv4/IPv6, reglas filter y raw, bloqueo de salida, registro y colocación de reglas."
---
...
<ConfigOption path="firewall.ipv4.enabled" />
...
<ConfigOption path="firewall.ipv4.address_list" />
...
<ConfigOption path="firewall.ipv6.enabled" />
...
<ConfigOption path="firewall.ipv6.address_list" />
...
<ConfigOption path="firewall.filter.enabled" />
...
<ConfigOption path="firewall.filter.chains" />
...
<ConfigOption path="firewall.raw.enabled" />
...
<ConfigOption path="firewall.raw.chains" />
...
<ConfigOption path="firewall.deny_action" />
...
<ConfigOption path="firewall.rule_placement" />
...
<ConfigOption path="firewall.comment_prefix" />
...
<ConfigOption path="firewall.log" />
...
<ConfigOption path="firewall.log_prefix" />
...
<ConfigOption
  path="firewall.block_input.interface"
  note="todas las interfaces"
/>
...
<ConfigOption
  path="firewall.block_input.interface_list"
  note="todas las interfaces"
/>
...
<ConfigOption path="firewall.block_output.enabled" />
...
<ConfigOption path="firewall.block_output.interface" />
...
<ConfigOption path="firewall.block_output.interface_list" />
...
<ConfigOption path="firewall.reject_with" />
...
<ConfigOption
  path="firewall.filter.connection_state"
  note="coincide con todos"
/>
...
<ConfigOption path="firewall.block_input.whitelist" />
...
:::note[Este listado es un ejemplo con todas las opciones activadas]
Es un ejemplo trabajado con whitelist, `forward`, `output` y logging activados — no una configuración de fábrica. Además omite las reglas passthrough de conteo que `metrics.track_processed` crea por defecto. Para la lista autoritativa y generada de todas las reglas que escribe el demonio, consulta [Reglas que escribe el demonio](/cs-routeros-bouncer/es/architecture/firewall-rules/), que se renderiza desde el mismo módulo del que se lee el código.
:::
