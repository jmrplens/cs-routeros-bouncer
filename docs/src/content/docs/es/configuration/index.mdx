...
| `mikrotik.address`        | `MIKROTIK_HOST`            | _(obligatorio)_          | Dirección de la API de RouterOS (`host:port`)  |
| `mikrotik.username`       | `MIKROTIK_USER`            | _(obligatorio)_          | Usuario de la API                              |
...
| `logging.file`                   | `LOG_FILE`                       |                   | Se acepta pero no está implementado; los logs siempre van a stderr         |
