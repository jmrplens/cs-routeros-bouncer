...
router, leyendo `/proc/stat` del kernel compartido a 10 Hz — y `/proc/meminfo`
una vez por segundo, porque la memoria no se mueve a 100 ms en esta carga — y
...
a Loki. `capture` extrae cualquier ventana como CSV; `plot` redibuja la gráfica
de arriba desde el dataset versionado. Cualquier verbo con `-h` muestra la
lista completa de flags.

`uninstall` elimina exactamente lo que `install` creó y nada más: cada objeto
se selecciona por el comentario exacto que `install` escribió, nunca por un
patrón — la tabla de address-lists que toca es la misma en la que el bouncer
guarda su estado de bloqueo, así que una coincidencia por subcadena ahí nunca
fue aceptable — más el fichero de imagen subido. Después vuelve a ejecutar sus
propias comprobaciones de propiedad y falla, nombrando el paso, si queda algo.
Un montaje hecho a mano que comparta un nombre queda intacto por ambos lados:
`install` se niega a continuar y nombra la colisión — no construye sobre
objetos que no creó y que después no eliminaría — y `uninstall` no lo posee y
lo ignora. El envlist y la imagen subida, que RouterOS no puede etiquetar, se
firman con una entrada `PERFMON_TAG` en el envlist y solo se eliminan mientras
ese marcador exista.

Dos cosas del despliegue son deliberadas y conviene saberlas antes de
confiar en él. La raíz del contenedor vive en la tmpfs del router por defecto
y se registra con `start-on-boot=no`, así que **no** sobrevive a un reinicio —
un instrumento no debe dejar rastro en un dispositivo que solo tomaba
prestado; `install` es idempotente y rápido de repetir. Y el instalador crea
dos pertenencias a listas del firewall porque el firewall por defecto de
RouterOS se come el tráfico del contenedor en silencio sin ellas: la regla raw
`drop the rest (in-interface-list=!LAN)` descarta cada paquete de un veth que
no esté en la lista de interfaces `LAN`, y `drop local if not from default IP
range` coincide con un origen fuera de la address-list `LANs`. El síntoma es
que el ICMP al contenedor funciona mientras su TCP saliente no genera entrada
de conntrack ni contador de masquerade; el sniffer no muestra nada. Ambas
pertenencias se crean y se eliminan con el resto.
