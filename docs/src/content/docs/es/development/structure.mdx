...
Medir el propio router — CPU por core y RAM a 10 Hz, leídas desde dentro del dispositivo — ya no se hace desde este repositorio. Lo hacía `cmd/perfmon` hasta que se convirtió en [mikroscope](https://github.com/jmrplens/mikroscope), que es donde vive ese trabajo ahora; [Benchmarking](/cs-routeros-bouncer/es/development/benchmarking/) tiene los comandos.
