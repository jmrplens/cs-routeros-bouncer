---
title: Arquitectura
description: "Cómo cs-routeros-bouncer conecta la inteligencia de amenazas de CrowdSec con los firewalls MikroTik: componentes, flujo de datos, pool, caché y adición masiva."
...
flowchart LR
    ...
    CS[LAPI de CrowdSec] -. "decisiones (lectura)" .-> B[cs-routeros-bouncer]
    B -->|"API de RouterOS (escritura)"| R[Router MikroTik]
    P[Prometheus] -. "/metrics (lectura)" .-> B
    H[Health Checks] -. "/health (lectura)" .-> B
...
<ThemeImage
  ...
  alt="El dashboard de Grafana incluido con cs-routeros-bouncer, con paneles de decisiones activas, ritmo de decisiones por origen, contadores de tráfico del firewall y salud del sistema RouterOS."
  caption="La rama /metrics, ya renderizada: grafana/dashboard.json en el repositorio."
/>
...
Tres canales llevan esa distinción, para que el diagrama siga leyéndose en escala de grises, con cualquier tipo de deficiencia en la visión del color y sobre papel: el patrón de la línea, después el color y, por último, una leyenda dibujada dentro de cada diagrama con esas mismas dos líneas. La convención está escrita en `docs/src/styles/diagram.css`; los colores salen de `docs/src/styles/theme.css` en tiempo de compilación, así que los diagramas siguen la paleta en lugar de fijar la suya.
