...
Leaving `origins` empty accepts all origins, including `CAPI` and named list origins. Use `lists:*` as the wildcard pattern for `lists:<name>` entries when setting `origins` or `CROWDSEC_ORIGINS`. See [CAPI Blocklists](/cs-routeros-bouncer/configuration/capi-blocklists/) for scale and router sizing guidance.
