...
Where to place new firewall rule blocks:

- `top` — at position 0, retrying lower positions if RouterOS refuses to move before dynamic/built-in rules
- `bottom` — at the end of the RouterOS firewall menu
- `position` — before a zero-based RouterOS `print` position
- `before_comment` — before a rule whose comment matches `comment`
- `after_comment` — after a rule whose comment matches `comment`
...
- `FIREWALL_RULE_PLACEMENT_COMMENT`
- `FIREWALL_RULE_PLACEMENT_COMMENT_MATCH`
- `FIREWALL_RULE_PLACEMENT_POSITION`
- `FIREWALL_RULE_PLACEMENT_FALLBACK`
