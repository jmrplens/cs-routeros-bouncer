---
title: Firewall
description: "Configure how cs-routeros-bouncer blocks traffic on MikroTik: IPv4/IPv6 lists, filter and raw rules, output blocking, logging, and rule placement."
---

import { Badge } from "@astrojs/starlight/components";
import ConfigOption from "../../../components/ConfigOption.astro";

Settings for IPv4/IPv6 blocking, [RouterOS firewall](https://help.mikrotik.com/docs/spaces/ROS/pages/250708066/Firewall) rule creation, and output blocking.
...
For the environment variable form, comma-separated entries may be written as
...
Like filter chains, comma-separated environment values tolerate surrounding
whitespace around each chain name.
...
<ConfigOption path="firewall.block_input.interface" note="all interfaces" />
...
:::note
If both `interface` and `interface_list` are set, both are applied to the rule (RouterOS evaluates them as AND), the same as for `block_input`.
:::
...
<ConfigOption path="firewall.filter.connection_state" note="matches all" />
...
:::note[This listing shows an all-options-enabled example]
It is a worked example with whitelist, `forward`, `output` and logging all turned on — not a stock configuration. It also omits the passthrough counting rules that `metrics.track_processed` creates by default. For the authoritative, generated list of every rule the daemon writes, see [Rules the daemon writes](/cs-routeros-bouncer/architecture/firewall-rules/), which renders from the same module the code is read from.
:::
