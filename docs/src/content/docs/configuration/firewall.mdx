...
| `position`       | `position`                                               | Not applicable     | Yes, retries lower positions when the target cannot be used     | Place at a RouterOS `print` position                     |
...
| `position`      |         | `position`                        | Zero-based RouterOS `print` position. Negative values are invalid.                                                                                                   |
...
The same structured object can be used under `firewall.ipv4.rule_placement` and `firewall.ipv6.rule_placement` for YAML-only protocol overrides. These overrides inherit unspecified fields from the global placement.
...
`position` uses zero-based RouterOS `print` numbering. If the configured position is greater than the number of existing non-bouncer rules, the block is appended at the bottom. `fallback` is ignored for `strategy: "position"`.
...
In this example, IPv4 rules are placed before the IPv4 anchor. IPv6 raw rules are appended at the bottom, while IPv6 filter rules are placed after `IPv6 filter anchor`.
...
- `FIREWALL_RULE_PLACEMENT_POSITION` - zero-based numeric position for `position`
