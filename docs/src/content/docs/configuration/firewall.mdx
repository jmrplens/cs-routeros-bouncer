...
| `position`       | `position`                                               | No, ignored if set | Yes, retries lower positions when the target cannot be used     | Place at a RouterOS `print` position                     |
...
| `filter`        |         | Structured config only            | Override the global placement for `/ip firewall filter` and `/ipv6 firewall filter`, including optional output rules.           |
...
- `FIREWALL_RULE_PLACEMENT` - global strategy (`top`, `bottom`, `position`, `before_comment`, or `after_comment`)
...
**Env:** `FIREWALL_INPUT_INTERFACE` · **Default:** — (all interfaces)
...
**Env:** `FIREWALL_INPUT_INTERFACE_LIST` · **Default:** — (all interfaces)
...
**Env:** `FIREWALL_OUTPUT_INTERFACE` · **Default:** —
...
**Env:** `FIREWALL_OUTPUT_INTERFACE_LIST` · **Default:** —
...
**Env:** `FIREWALL_INPUT_WHITELIST` · **Default:** —
...
**Env:** `FIREWALL_OUTPUT_PASSTHROUGH_V4` / `FIREWALL_OUTPUT_PASSTHROUGH_V6` · **Default:** —
...
**Env:** `FIREWALL_OUTPUT_PASSTHROUGH_V4_LIST` / `FIREWALL_OUTPUT_PASSTHROUGH_V6_LIST` · **Default:** —
