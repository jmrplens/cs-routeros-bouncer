...
Community blocklists (CAPI) can contain 20,000+ IPs. If your router has limited resources or you only want to block IPs detected on your network, local-only mode is recommended. Local-only (~1,500 IPs) reconciles in ~9 s with ~14% Router CPU peak. Full CAPI (~25,000 IPs) takes ~2 min 50 s with ~23% Router CPU peak. After reconciliation completes, Router CPU should settle back near normal idle/traffic levels.
:::
