...
Community blocklists (CAPI) can contain 20,000+ IPs. If your router has limited resources or you only want to block IPs detected on your network, local-only mode is recommended. In the RB5009 benchmark environment, local-only mode typically reconciled ~1,500 IPs in ~9 s with ~14% Router CPU peak, while full CAPI with around ~25,000 IPs took ~2 min 50 s with ~23% Router CPU peak. Results vary by hardware, RouterOS version, address-list size, and network workload. After reconciliation completes, Router CPU should settle back near normal idle/traffic levels.
:::
