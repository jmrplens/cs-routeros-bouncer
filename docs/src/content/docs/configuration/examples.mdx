---
title: Examples
description: "Complete cs-routeros-bouncer configuration examples: minimal, full protection, performance tuning, CAPI control, rule placement, TLS, and Docker Compose."
---
...
:::
  ...
  <TabItem label="Performance Tuning">

Balanced settings for larger local lists or partial CAPI usage on capable RouterOS hardware.
...
Use `pool_size: 1` or `2` on small routers. See [Performance Tuning](/cs-routeros-bouncer/configuration/performance-tuning/) for sizing guidance.

  </TabItem>
  <TabItem label="CAPI Controlled">
...
Start with local-only mode on small routers, then enable CAPI after checking reconciliation time and CPU. See [CAPI Blocklists](/cs-routeros-bouncer/configuration/capi-blocklists/).
