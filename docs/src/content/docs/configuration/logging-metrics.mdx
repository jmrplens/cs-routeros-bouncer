...
Under the hardened systemd unit written by `setup`, `ReadWritePaths=/var/log` is the only writable location, so a path outside `/var/log` will fail even if it exists. Relative paths resolve against `/` under systemd, so always give an absolute path.
:::
