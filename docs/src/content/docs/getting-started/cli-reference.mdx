...
The `cs-routeros-bouncer` binary has a small command surface: it either runs the bouncer, prints version/help information, or performs systemd setup/removal tasks.
...
| `-purge` | `false`                              | Also remove the config directory |
...
There is no `run` subcommand and no `version` subcommand. Use `cs-routeros-bouncer -c ...` to run and `cs-routeros-bouncer -version` to print build information.
