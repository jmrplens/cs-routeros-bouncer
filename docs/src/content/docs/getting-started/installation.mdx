...
The Docker image runs `cs-routeros-bouncer -c /etc/cs-routeros-bouncer/config.yaml` by default, so mount your file at that path:
...
`uninstall` also accepts `-bin` if the binary was installed outside `/usr/local/bin/cs-routeros-bouncer`.
